...
int MySocket::GetData(char * buff) {
	...
	char* rawbuff = new char[this->MaxSize];
	char* ptr = rawbuff;
	int size = 0;

	if (this->connectionType == TCP)
	{
		recv(this->ConnectionSocket, rawbuff, DEFAULT_SIZE, 0);
	}
	else if (this->connectionType == UDP)
	{
		int addr_len = sizeof(SvrAddr);
		recvfrom(this->ConnectionSocket, rawbuff, DEFAULT_SIZE, 0, (struct sockaddr*)&SvrAddr, &addr_len);
	}

	memcpy(buff, rawbuff, DEFAULT_SIZE);
